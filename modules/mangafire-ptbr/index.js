...
(() => {
  ...
  function protectedFetchActionScript(requests) {
    ...
    return `(() => {
      ...
      for (const spec of ${JSON.stringify(specs)}) {
        ...
        if (!response.ok) {
          throw new Error(payload.message || ('MangaFire HTTP ' + response.status));
        }
  ...
  async function protectedPageJSON(requests) {
    const snapshot = await globalThis.pagev2({
      ...
      returnScript:
        "globalThis.__synthetiqMangaFireProtectedResult || JSON.stringify({ ok: false, error: 'MangaFire protected request did not finish.' })",
  ...
  function ensureSandboxGlobals() {
    ...
    if (
      ...
      typeof navigator.appCodeName !== "string" ||
    ...
    if (typeof document === "undefined") {
      ...
      defineGlobal("cancelAnimationFrame", (identifier) =>
        clearTimeout(identifier)
      );

      defineGlobal("requestIdleCallback", (callback) =>
        setTimeout(
          () =>
            callback({
              didTimeout: false,
      ...
      defineGlobal("cancelIdleCallback", (identifier) =>
        clearTimeout(identifier)
      );
  ...
  async function loadSigner(forceRefresh = false) {
    ...
    signerPromise = (async () => {
      ...
      if (!homeResponse || homeResponse.error) {
        throw new Error(
          homeResponse && homeResponse.error
            ? String(homeResponse.error)
            : "MangaFire home page request failed."
        );
      }

      if (!homeResponse.ok) {
        throw new Error(
          `MangaFire home page returned HTTP ${
            homeResponse.status || "error"
          }.`
        );
      }
      ...
      const polyfillMatch = html.match(
        /href="([^"]*\/polyfill-[^"]+\.js)"/
      );

      if (!configMatch || !buildMatch || !polyfillMatch) {
        throw new Error(
          "MangaFire protection configuration was not found on the home page."
        );
      }

      const polyfillResponse = await globalThis.fetchv2(
        polyfillMatch[1],
        {
          Accept: "*/*",
        }
      ...
      if (
        !polyfillResponse ||
        polyfillResponse.error ||
        !polyfillResponse.ok
      ) {
        throw new Error(
          "MangaFire protection module could not be downloaded."
        );
      }

      const polyfillSource = String(
        polyfillResponse.body || ""
      );

      const exportsMatch = polyfillSource.match(
        /export\s*\{([^}]*)\}/
      );
      ...
      const wrappedSource =
        polyfillSource.replace(/export\s*\{[^}]*\};?/, "") +
      ...
      const signer =
        globalThis.__synthetiqMangaFireSigner;

      if (!signer || typeof signer.a !== "function") {
        throw new Error(
          "MangaFire protection module changed."
        );
      }
  ...
  async function signRequestURL(url, signer) {
    ...
    if (!interceptors.length) {
      throw new Error(
        "MangaFire request interceptor was not registered."
      );
    }
  ...
  async function headlessPageJSON(url, options = {}) {
    if (typeof globalThis.fetchv2 !== "function") {
      throw new Error(
        "MangaFire requires the fetchv2 bridge."
      );
    }

    let signer = await loadSigner();

    let signedURL = await signRequestURL(
      url,
      signer
    );
    ...
    if (
      ...
      if (
        ...
        signedURL = await signRequestURL(
          url,
          signer
        );
    ...
    if (!response) {
      throw new Error(
        "MangaFire API returned no response."
      );
    }
    ...
    if (!response.ok) {
      ...
      const message =
        rejected &&
        (rejected.message || rejected.error)
          ? String(
              rejected.message ||
                rejected.error
            )
          : `MangaFire API returned HTTP ${
    ...
    if (!payload) {
      throw new Error(
        "MangaFire API returned no JSON payload."
      );
    }
  ...
  async function pagev2JSON(url, options = {}) {
    ...
    for (
      ...
      try {
          ...
          for (
            ...
            payload = parseJSON(
              snapshot.events[index] &&
                snapshot.events[index].body
            );
          }
        }

        if (!payload && snapshot) {
          payload = JSONFromHTML(
            snapshot.html
          );
        }
        ...
        if (!payload) {
          throw new Error(
            "MangaFire pagev2 returned no JSON. The source may be challenged or unavailable."
          );
        }
        ...
        if (/token/i.test(APIMessage)) {
          ...
          return (
            await protectedPageJSON([url])
          )[0];
        }

        if (payload.error) {
          throw new Error(
            `MangaFire API error: ${String(
              payload.error
            )}`
          );
        }
  ...
  async function pageJSON(url, options = {}) {
    ...
    if (
      typeof globalThis.pagev2 ===
      "function"
    ) {
      transports.push(pagev2JSON);
    }
    ...
    for (
      ...
      for (const transport of transports) {
        try {
          ...
          if (/token/i.test(APIMessage)) {
            throw new Error(
              `MangaFire API requires protection refresh: ${APIMessage}`
            );
          }
  ...
  function titlePath(value) {
    ...
    throw new Error(
      "Invalid MangaFire title identifier."
    );
  }

  function titleHID(value) {
    return titlePath(value)
      .replace("/title/", "")
      .split("-")[0];
  }
  ...
  function chapterID(value) {
    ...
    const match =
      input.match(
        /(?:https:\/\/mangafire\.to)?\/?title\/[^/?#]+\/chapter\/([0-9]+)/i
      ) ||
      input.match(
        /(?:https:\/\/mangafire\.to)?\/?chapter\/([0-9]+)/i
      );
    ...
    throw new Error(
      "Invalid MangaFire chapter identifier."
    );
  }

  function addExcludedGenres(params) {
    EXCLUDED_GENRE_IDS.forEach((id) =>
      params.push([
        "genres_ex[]",
        String(id),
      ])
  ...
  function normalizeSearchQuery(query) {
    ...
    if (
      ...
      const excludeTags = Array.isArray(
        query.excludeTags
      )
        ? query.excludeTags
            .map((t) =>
  ...
  function searchURL(query, page) {
    ...
    return `${BASE_URL}/api/titles?${queryString(
      params
    )}`;
  }
  ...
  function isExplicitlyExcluded(item) {
    ...
    return groups.some(
      (group) =>
        Array.isArray(group) &&
        group.some((entry) =>
          EXCLUDED_GENRE_IDS.includes(
            Number(entry && entry.id)
          )
        )
    );
  }
  ...
  function detailsObject(
    ...
    if (
      ...
      throw new Error(
        "MangaFire details are missing or excluded by the module content policy."
      );
    }

    const path = titlePath(
      item.url || fallback
    );
    ...
    const genres = [
      ...groupTitles(item.genres),
      ...groupTitles(item.themes),
      ...groupTitles(
        item.demographics
      ),
    ];
  ...
  async function extractDetails(id) {
    ...
    return detailsObject(
      await pageJSON(
        `${BASE_URL}/api/titles/${encodeURIComponent(
          hid
        )}`
      ),
      id
  ...
  async function canonicalTitlePath(
    id,
    hid
  ) {
  ...
  function chapterURL(hid, page) {
    ...
    return `${BASE_URL}/api/titles/${encodeURIComponent(
      hid
    )}/chapters?${queryString(params)}`;
  }
  ...
  async function extractChapters(id) {
    ...
    const path =
      await canonicalTitlePath(
        id,
        hid
      );

    const first = await pageJSON(
      chapterURL(hid, 1)
    );
    ...
    if (
      ...
      throw new Error(
        `MangaFire chapter list exceeds the ${MAX_CHAPTER_PAGES}-page safety limit.`
      );
    }
    ...
    if (
      ...
      responses.push(
        ...(await protectedPageJSON(
          remainingURLs
        ))
      );

      if (
        typeof globalThis.reportProgress ===
        "function"
      ) {
        await globalThis.reportProgress({
          stage: "chapters",
          completed: lastPage,
          total: lastPage,
        });
    ...
    for (const response of responses) {
      for (const item of Array.isArray(
        ...
        const href =
          `${BASE_URL}${path}/chapter/${remoteID}`;
  ...
  async function extractImages(id) {
    ...
    const payload = await pageJSON(
      `${BASE_URL}/api/chapters/${encodeURIComponent(
        remoteID
      )}`
    );
    ...
    if (!pages.length) {
      throw new Error(
        "MangaFire chapter returned no readable image entries."
      );
    }
